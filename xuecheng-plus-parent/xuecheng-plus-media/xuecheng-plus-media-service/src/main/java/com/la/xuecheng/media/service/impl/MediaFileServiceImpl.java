...
public class MediaFileServiceImpl implements MediaFileService {
   ...
   public UploadFileResultDto upLoadFile(Long companyId, UploadFileParamsDto uploadFileParamsDto, String localFilePath) {
       ...
       //非事务方法调用事务方法，事务无法被控制，自己注入自己（代理对象），并调用自己的方法
       MediaFiles mediaFiles = currentProxy.addMediaFilesToDb(companyId, fileMd5, uploadFileParamsDto, bucket_mediafiles, objectName);
       if(mediaFiles == null){
           XuechengPlusException.cast("图片信息入库失败");
       }
   ...
   public boolean addMediaFilesToMinIO(String mimeType, String localFilePath, String bucket, String objectName){
    ...
    public RestResponse uploadChunk(String fileMd5, int chunk, String localChunkFilePath) {
        ...
        //将分块文件上传到minio
        boolean b = addMediaFilesToMinIO(mimeType, localChunkFilePath, bucket_video, chunkFilePath);
        if(!b){
